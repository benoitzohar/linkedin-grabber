...
function sanitizeName(name) {
  ...
  return normalized.replace(/^(\(\d+\)\s*)+/u, "").trim();
}
...
export async function extractPublicProfileFromDocument(expectedProfileType = "auto") {
  ...
  const extractName = () => {
    ...
    for (const selector of selectors) {
      ...
      const text = normalize(node.getAttribute?.("alt") || node.textContent || "");
      if (text) {
        return text;
      }
    }
  ...
  const scanDocumentForPublicUrl = () => {
    const highSignalNodes = document.querySelectorAll(
      "[data-test-personal-info-profile-link], [data-test-personal-info-profile-link-text], [data-test-public-profile-link], [data-test-public-link]"
    );
  ...
  const existingPublicUrl = canonicalPublicUrlFromRaw(scanDocumentForPublicUrl());
  ...
  const publicLinkControl = findPublicLinkControl();
  ...
  const copyLinkControl = findClickableByText([
    "copy link",
    "copy profile link",
    "copier le lien",
